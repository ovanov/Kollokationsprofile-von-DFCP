...
def preprocess_text(filename: str) -> List:
    ...
    this list contains all the words that come after a w-elemnt dfcp
    """
    ...
    with open(filename, 'r') as annis:
        for line in annis:
            if re.search(r"\d+\.", line) != None: # take only the needed lines
                if re.search(r"\d+\.\ttok\s\sw\w+\sdass?", line) != None: # check if there is a w-elemnt 
                    line = re.sub(r'\d+\.\ttok\s\sw\w+\sdass?', "", line) # strip that part off 
                    listed_words.append(line.split())
                    # print(line.split())
                elif re.search(r"\d+\.\ttok\s\sw\w+\s\w+\sdass?", line) != None: # check if there is a w-elemnt and phrasal copmponent
...
def main():
    ...
    preprocessed_text = preprocess_text('./w_pos.txt') # read file (txt) : change file path, if needed 
    collocations = count_collocations(preprocessed_text)
    save_as_csv(collocations, './w_pos.csv') # write csv from collovations (list[list[(tuple)]])
