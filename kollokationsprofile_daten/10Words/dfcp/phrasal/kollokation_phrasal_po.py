...
def main():
    ...
    preprocessed_text = preprocess_text('./pos.txt') # read file ( txt ) : change file path, if needed
    collocations = count_collocations(preprocessed_text)
    save_as_csv(collocations, './prasal_pos.csv') # write csv from collocations (list[list[(tuple)]])
